...
def name(): #To ask for user's name
    global name
    while True:
        name = input("Enter Your Name : ").capitalize() #Capitalizes first name
        if name.replace(' ','').isalpha(): #Ignores spaces
            break
...
def inst(): #To ask if user needs instructions
    while True:
        ...
        if inst == 'No' or inst == 'no' or inst == 'b' or inst == 'B' or inst == 'n' or inst == 'N'or inst == 'nO' or inst == 'NO'or inst == 'nO': #If user does not know how to play the quiz and requires instructions
            ...
            print("Please enter yes or no") #Error message

def status(): #To ask if the user is ready to play
    while True:
...
def gamerounds(): #To ask the user how many rounds they want to play
    global g ,total
    while True:
        try:
...
inst() #Calling the instructions function
status() #Calling the ready status function
gamerounds() #Calling the gamerounds function


while g>0: #While gamerounds are more than zero
    data = mathquiz[0]
    q = data[0]
    data =data[1]
    answer = data['answer']
    ...
    print(q) #Print the questions
    print(option) #Print the options
    while True:
        answers = input("Please enter your answer here : ").lower().replace(' ','') #To ingore spaces
        if answers =='a' or answers == 'b' or answers == 'c' or answers == 'd': #Acceptable answers
            if answers == answer:
                print("********************************************************************************************************************")
...
print("")
print("**************************************************** QUIZ SUMMARY ****************************************************") #End of quiz summary
